...
2. Select a workflow (e.g., **Mood Topic Poem**, **Codebase Health Check**).
3. Click **▶ Run**.
4. Provide any requested input.

Events stream live while the workflow runs, and a full trace is saved for later review.

## Where Workflows and Runs Are Stored

- **Workflows**: `~/.copilot-console/workflows/` (Windows: `C:\Users\<username>\.copilot-console\workflows\`)
- **Workflow runs** (history + working dirs): `~/.copilot-console/workflow-runs/`
...
| File | What it shows |
|---|---|
| `mood-topic-poem.yaml` | The basics — `Question` with choices (mood), free-text input (topic), an agent invocation that writes a poem. No Power Fx. |
| `workflow-feature-advanced.yaml` | Branching + looping — `If`, `Switch`, `Foreach`. **Uses Power Fx.** |
...
Check that the referenced agents exist in your Agent Library. A sticky red toast appears in the run view with the error from the Agent Framework. The run row in the editor's history table also shows `failed` with the error in the error column. For known runtime caveats, see [Agent Framework SDK Patches](AF_SDK_PATCHES.md).
