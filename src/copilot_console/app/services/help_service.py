...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "2. Use the `view` tool to list the docs directory and read the relevant file(s). "
        "If you are not sure which file to read, list `guides/` first and pick the best match.\n\n"
        ...
        "4. Cite which doc the answer came from, e.g. \"(from guides/MCP-SERVERS.md)\".\n\n"
        "## Guidelines\n\n"
        "- Always read the docs before answering — don't guess or fabricate.\n"
        "- If the docs don't cover the topic, say so clearly and suggest where to look "
        "(e.g. GitHub issues, the slash-command palette, or the Settings panel).\n"
